...
class ModelList(list):
    ...
    def load_folds(cls, model_dir):
        ...
        try:
            model_list.all_results = pd.read_csv(os.path.join(model_dir, 'predictions.csv'),
                                                 index_col=0)
            model_list.all_stats = pd.read_csv(os.path.join(model_dir, 'predict_stats.csv'),
                                               index_col=0)
        except:
            pass
        for fold_dir in fold_list:
            model_list.append(load_model(fold_dir))
        return model_list

    @classmethod
    def load_model_set(cls, model_dir, num_runs=None):
        ...
        try:
            ...
            raise FileNotFoundError(f'Model parameters missing: {model_file}')
        run_list = glob.glob(os.path.join(model_dir, 'run*'))
        if num_runs:
        ...
        if len(run_list) == 0: # A single run, so load the model
            if model_params['splitFolds'] <= 1:
                ...
                return cls.load_folds(model_dir)
 
        model_name = model_params['modelName']
        model_list = cls(model_name, model_dir)
        model_list.params = model_params
        try: # Load predictions and statistics files if they exist
            model_list.run_stats = pd.read_csv(os.path.join(model_dir, 'stats_all.csv'),
                                               index_col=[0, 1])
            model_list.means = pd.read_csv(os.path.join(model_dir, 'stats_means.csv'),
                                           index_col=0)
            model_list.variances = pd.read_csv(os.path.join(model_dir, 'stats_vars.csv'),
                                               index_col=0)
            model_list.all_stats = pd.read_csv(os.path.join(model_dir, 'ensemble_stats.csv'),
                                               index_col=0)
            model_list.all_results = pd.read_csv(os.path.join(model_dir, 'ensemble_preds.csv'),
                                                 index_col=0)
        except:
            model_list.all_results = None
        for run_dir in run_list:
            if model_params['splitFolds'] <= 1:
                model_list.append(load_model(run_dir))
            else:
                model_list.append(cls.load_folds(run_dir))
        return model_list

    @classmethod
    def load_experiment(cls, model_dir, experiment_only=True, num_tests=None):
        """Loads an experiment
        ...
        if len(param_files) == 0:
            if experiment_only:
                ...
                models = cls.load_model_set(model_dir)
        ...
        model_list.params = model_params
        test_dirs = glob.glob(os.path.join(model_dir, 'test*'))
        if num_tests:
            test_dirs = test_dirs[:num_tests]
        for test_dir in test_dirs:
            try:
                model_list.append(cls.load_model_set(test_dir))
            except:
                break
        return model_list
