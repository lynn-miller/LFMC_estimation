...
if __name__ == '__main__':
    ...
    era5_csv = os.path.join(common.DATASETS_DIR, 'era5_365days.csv')
    ...
    experiment = ExperimentParams({
        'name': 'conus_base_models',
        'description': 'Build CONUS source models',
        'tests': [
            {'testName': 'CONUS all data', 'auxAugment': True},
            {'testName': 'CONUS yearly models',
             'auxAugment': True,
             'splitMethod': 'byYear', 'splitYear': 2014, 'yearFolds': 4},
            {'testName': 'CONUS yearly models - no target normalise',
             'targetNormalise': False, 'auxAugment': True,
             'splitMethod': 'byYear', 'splitYear': 2014, 'yearFolds': 4},
            {'testName': 'CONUS yearly models - no aux augment',
             'auxAugment': False,
             'splitMethod': 'byYear', 'splitYear': 2014, 'yearFolds': 4},
            {'testName': 'CONUS yearly models - no target normalise or augment',
             'targetNormalise': False, 'auxAugment': False,
             'splitMethod': 'byYear', 'splitYear': 2014, 'yearFolds': 4},
        ],
        'restart': None, 
        'rerun': 1,
    ...
    model_params['gpuList'] = [2, 3]     # List of GPUs to use
    model_params['gpuMemory'] = 4500     # GPU memory for each worker
