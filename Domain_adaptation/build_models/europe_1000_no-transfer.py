...
if __name__ == '__main__':
    ...
    # =============================================================================
    random.seed(2023)
    seeds = random.choices(range(1000, 10000), k=50)
    random.seed(1234)
    model_seeds = [random.choices(seeds, k=20) for _ in range(20)]
    
    folds_dir = os.path.join(common.MODELS_DIR, 'europe_gen-folds')
    experiment = ExperimentParams({
        'name': 'europe_1000_no-transfer',
        'description': 'Europe: no pretraining; 1000 training samples',
        'tests': [
            {'testName': 'Ensemble 1', 'randomSeed': 9013, 'seedList': model_seeds[0],
             'loadFolds': os.path.join(folds_dir, 'test0')},
            {'testName': 'Ensemble 2', 'randomSeed': 1815, 'seedList': model_seeds[1],
             'loadFolds': os.path.join(folds_dir, 'test1')},
            {'testName': 'Ensemble 3', 'randomSeed': 5313, 'seedList': model_seeds[2],
             'loadFolds': os.path.join(folds_dir, 'test2')},
            {'testName': 'Ensemble 4', 'randomSeed': 3945, 'seedList': model_seeds[3],
             'loadFolds': os.path.join(folds_dir, 'test3')},
            {'testName': 'Ensemble 5', 'randomSeed': 3632, 'seedList': model_seeds[4],
             'loadFolds': os.path.join(folds_dir, 'test4')},
            {'testName': 'Ensemble 6', 'randomSeed': 3875, 'seedList': model_seeds[5],
             'loadFolds': os.path.join(folds_dir, 'test5')},
            {'testName': 'Ensemble 7', 'randomSeed': 1782, 'seedList': model_seeds[6],
             'loadFolds': os.path.join(folds_dir, 'test6')},
            {'testName': 'Ensemble 8', 'randomSeed': 1393, 'seedList': model_seeds[7],
             'loadFolds': os.path.join(folds_dir, 'test7')},
            {'testName': 'Ensemble 9', 'randomSeed': 3708, 'seedList': model_seeds[8],
             'loadFolds': os.path.join(folds_dir, 'test8')},
            {'testName': 'Ensemble 10', 'randomSeed': 2914, 'seedList': model_seeds[9],
             'loadFolds': os.path.join(folds_dir, 'test9')},
            {'testName': 'Ensemble 11', 'randomSeed': 4522, 'seedList': model_seeds[10],
             'loadFolds': os.path.join(folds_dir, 'test10')},
            {'testName': 'Ensemble 12', 'randomSeed': 3368, 'seedList': model_seeds[11],
             'loadFolds': os.path.join(folds_dir, 'test11')},
            {'testName': 'Ensemble 13', 'randomSeed': 6379, 'seedList': model_seeds[12],
             'loadFolds': os.path.join(folds_dir, 'test12')},
            {'testName': 'Ensemble 14', 'randomSeed': 3009, 'seedList': model_seeds[13],
             'loadFolds': os.path.join(folds_dir, 'test13')},
            {'testName': 'Ensemble 15', 'randomSeed': 3806, 'seedList': model_seeds[14],
             'loadFolds': os.path.join(folds_dir, 'test14')},
            {'testName': 'Ensemble 16', 'randomSeed': 6579, 'seedList': model_seeds[15],
             'loadFolds': os.path.join(folds_dir, 'test15')},
            {'testName': 'Ensemble 17', 'randomSeed': 4075, 'seedList': model_seeds[16],
             'loadFolds': os.path.join(folds_dir, 'test16')},
            {'testName': 'Ensemble 18', 'randomSeed': 1056, 'seedList': model_seeds[17],
             'loadFolds': os.path.join(folds_dir, 'test17')},
            {'testName': 'Ensemble 19', 'randomSeed': 5261, 'seedList': model_seeds[18],
             'loadFolds': os.path.join(folds_dir, 'test18')},
            {'testName': 'Ensemble 20', 'randomSeed': 4752, 'seedList': model_seeds[19],
    ...
    model_params['samplesFilter'] = {'apply': 'train', 'method': 'random', 'params': [1000]}
    ...
    model_params['splitColumn'] = 'Group2' # 'Site'
    ...
    model_params['gpuMemory'] = 1024    # GPU memory for each worker
